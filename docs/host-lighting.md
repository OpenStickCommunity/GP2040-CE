...
`SET_BUTTONS` colours every light belonging to the control, so one entry
lights both Up buttons, and `FILL`'s buttons scope reaches every button light.

Page 2 reports one LED range per control regardless. A host that needs each
of a control's lights individually reads page 5 - which lists every light and
names its owner - and colours them by ordinal with `SET_LIGHT` or by raw index
with `SET_RANGE`.
...
1 SWITCH, 2 PS3, 3 KEYBOARD, 4 PS4, 5 XBONE, 6 MDMINI, 7 NEOGEO, 8 PCEMINI,
9 EGRET, 10 ASTRO, 11 PSCLASSIC, 12 XBOXORIGINAL, 13 PS5, 14 GENERIC,
15 SWITCH_PRO, 16 P5GENERAL, 17 SINPUT); the config mode is entered via
`REBOOT_WEBCONFIG` instead.
...
Measured over 300 pings the median is 2.0 ms on both test boards.
...
waiting for that frame's `COMMIT` reply. Match each reply by its command byte
and sequence number and keep a small queue of recent replies; a host that
keeps only the newest reply drops acknowledgements it is waiting for and
...
simply discards the difference with nothing to tell it why.
...
Page 2 maps the eighteen canonical controls to LED ranges; it is not an
inventory of the board's lights. A board may carry lights on controls page 2
cannot name, and lights with no owning control. A control that owns several
lights is reported by the first of them. Where the page 1 light-table bit is
set, page 5 is authoritative.

`[6]` is the extent - the highest LED index in use, plus one - which is the
number a host sizes a frame buffer from. It is not the sum of the ranges
above it, which would miss the lights page 2 cannot name.

**Page 3 - animations**: `[3]` current index (0xFF = none selected),
`[4]` count. Pairs with `SET_ANIMATION`. The count is what the board actually
offers, which varies by board and by how the user has configured it - do not
assume a fixed number. `SET_ANIMATION` rejects an index at or above the
reported count with an invalid-argument status.

**Page 4 - per-light positions** (request `[3]=start entry`): `[3]` total,
`[4]` count in reply, then count x `[firstLED, gridX, gridY]`. A board whose
light registry is not yet populated reports zero entries; hosts fall back to
the layout enum. Page 4 is a projection of page 5's columns and
...
Read the stride from `[6]` rather than assuming 12. A record is never widened
in place; future per-light fields go on a new page.

Both flags are positive assertions: a set bit states the field is real, and a
clear bit claims nothing. Current firmware sets both on every record; fields
it cannot supply carry the sentinels above.
...
5. Poll page 1 periodically; if the fingerprint changes, re-read whichever of
   pages 2, 4, 5 and 6 you cache - one fingerprint certifies all four, and a
...
board renders at 40 Hz today. **Read the actual rate from page 1 `[18]`
...
firmware on the same board. The figures were taken at v1.0, before GP2040-CE
moved to its current LED pipeline, and have not been repeated since; the
staging and commit path that runs on the input core is the same code now as
then.
...
measured the same. No configuration measured slower than stock. Enabling the
add-on with no host connected costs nothing measurable, and with the add-on
disabled the lighting code never runs.
...
Measured on both test boards, streaming whole frames of `SET_RANGE` plus one
`COMMIT`:

| | round-trip median | 60 fps commits | unthrottled |
|---|---|---|---|
| 46 LEDs | 2.00 ms | 300/300, 0 lost | 125 fps |
| 16 LEDs | 2.00 ms | 300/300, 0 lost | 250 fps |

Both boards hold a full 60 fps with no lost acknowledgements. Unthrottled
throughput scales with the number of reports a frame needs: four for a 46-LED
board, two for a 16-LED board. Both figures exceed the render rate, so
...
Not a protocol version. The add-on now targets GP2040-CE's merged LED
pipeline only, and nothing on the wire changed. The lighting interface is
additionally available in SInput mode.
...
- **Host-supplied W is honoured on white-format chains.** Earlier firmware
  ...
  exactly as before. See White handling under Notes.
