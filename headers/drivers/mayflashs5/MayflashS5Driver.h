...
#ifndef _MAYFLASHS5_DRIVER_H_
#define _MAYFLASHS5_DRIVER_H_

#include "gpdriver.h"
#include "drivers/mayflashs5/MayflashS5Descriptors.h"
...
} MayflashS5AuthReport;

class MayflashS5Driver : public GPDriver {
public:
...
private:
    // Uncollected report is dropped after this (console pauses ~28 ms at auth).
    static constexpr uint64_t UPSTREAM_DROP_US = 100000;
    // Longer limit while the console is still enumerating.
    static constexpr uint64_t ENUMERATION_DROP_US = 2000000;
    // Reports taken since mount; this many means enumeration is done.
    static constexpr uint32_t CONSOLE_READY_REPORTS = 30;
    uint32_t reports_since_mount = 0;
    uint64_t upstream_wait_since_us = 0;
    bool upstream_dropping = false;
