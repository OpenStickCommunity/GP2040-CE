...
// The TGS developer document identifies Mayflash's F300/F500 family as its
// intended upstream "console" device. Keep the P5General PS5 report protocol,
// but identify this test build as the F300 rather than an unrelated adapter.
static const uint8_t mayflashf300_string_language[]     = { 0x09, 0x04 };
static const uint8_t mayflashf300_string_manufacturer[] = "Mayflash";
static const uint8_t mayflashf300_string_product[]      = "Arcade Fightstick F300";
static const uint8_t mayflashf300_string_version[]      = "1.0";

static const uint8_t *mayflashf300_string_descriptors[] __attribute__((unused)) =
{
    mayflashf300_string_language,
    mayflashf300_string_manufacturer,
    mayflashf300_string_product,
    mayflashf300_string_version
};

static const uint8_t mayflashf300_device_descriptor[] =
{
    0x12, 0x01,
    0x00, 0x02,
    0x00, 0x00, 0x00,
    0x40,
    0x79, 0x00, // VID 0x0079
    0x30, 0x18, // PID 0x1830
    0x00, 0x01,
    0x01, 0x02, 0x00,
    0x01
};
...
// The DualSense identity presented upstream. The PS5 binds the signature to the
// device it enumerated, so a DualSense-signed response has to come from a
// device that enumerates as a DualSense.
//
// The device descriptor advertises an 8-byte endpoint 0, so CMakeLists.txt sets
// CFG_TUD_ENDPOINT0_SIZE=8 to match. The two must change together.
...
static const uint8_t mayflashs5_device_descriptor[] =
{
    ...
    0x08,        // bMaxPacketSize0 8 (captured; see CFG_TUD_ENDPOINT0_SIZE)
...
// The 227-byte composite configuration descriptor: audio control,
// two audio streaming interfaces, and the HID gamepad interface.
// The HID interface is interface 3; audio streaming interfaces default to
// alternate setting 0, so no isochronous endpoints are active at startup.
static const uint8_t mayflashs5_full_configuration_descriptor[] = {
