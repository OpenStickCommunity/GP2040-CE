...
void ControllerHostListener::setup()
{
    _controller_host_enabled = false;
}

void ControllerHostListener::process()
{
...
void ControllerHostListener::mount(uint8_t dev_addr, uint8_t instance, uint8_t const* desc_report, uint16_t desc_len)
{
  _controller_host_enabled = true;
  tuh_vid_pid_get(dev_addr, &controller_vid, &controller_pid);
  // printf("HID device mounted.");
  // printf("PID: %x, VID: %x \r\n", controller_pid, controller_vid);
}

void ControllerHostListener::unmount(uint8_t dev_addr) {
  _controller_host_enabled = false;
  controller_pid = 0x00;
  controller_vid = 0x00;
...
void ControllerHostListener::report_received(uint8_t dev_addr, uint8_t instance, uint8_t const* report, uint16_t len)
{
  // if a hid device hasn't been mounted
  if ( _controller_host_enabled == false )
  {
    return;
  }
...
void ControllerHostListener::process_ctrlr_report(uint8_t dev_addr, uint8_t const* report, uint16_t len)
{
  ...
  switch(controller_pid)
  {
    ...
    case 0x05c4:
    {
      ...
      if (report_id == 1)
      {
        ...
        //prev_report.counter = controller_report.counter;

        if ( diff_report(&prev_report, &controller_report) )
        {
          // printf("x: %x\r\n", controller_report.GD_GamePadPointerX);
          // printf("y: %x\r\n", controller_report.GD_GamePadPointerY);
          // printf("Z: %x\r\n", controller_report.GD_GamePadPointerZ);
          // printf("rZ: %x\r\n", controller_report.GD_GamePadPointerRz);
...
uint16_t ControllerHostListener::map(uint8_t x, uint8_t in_min, uint8_t in_max, uint16_t out_min, uint16_t out_max) {
    return (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
}


//check if different than 2
bool ControllerHostListener::diff_than_2(uint8_t x, uint8_t y)
{
  return (x - y > 2) || (y - x > 2);
}

// check if 2 reports are different enough
bool ControllerHostListener::diff_report(dualshock4_t const* rpt1, dualshock4_t const* rpt2)
{
  bool result;

  // x, y, z, rz must different than 2 to be counted
  result = diff_than_2(rpt1->GD_GamePadPointerX, rpt2->GD_GamePadPointerX) || diff_than_2(rpt1->GD_GamePadPointerY , rpt2->GD_GamePadPointerY ) ||
           diff_than_2(rpt1->GD_GamePadPointerZ, rpt2->GD_GamePadPointerZ) || diff_than_2(rpt1->GD_GamePadPointerRz, rpt2->GD_GamePadPointerRz);
