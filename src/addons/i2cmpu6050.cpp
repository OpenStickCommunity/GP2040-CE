...
bool I2CMPU6050Input::available() {
    ...
    return (option.enabled && isValidPin(option.i2cSDAPin) && isValidPin(option.i2cSCLPin));
}

void I2CMPU6050Input::setup() {
    const MPU6050Options& options = Storage::getInstance().getAddonOptions().mpu6050Options;
    
    imu = new MPU6050(1, options.i2cSDAPin, options.i2cSCLPin, i2c1, 40000, I2C_MPU6050_ADDRESS);
    imu->init();
}
