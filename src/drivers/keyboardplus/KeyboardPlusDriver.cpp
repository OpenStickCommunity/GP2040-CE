...
void KeyboardPlusDriver::initialize() {
	keyboardReport = {
		.keycode = { 0 },
		.multimedia = 0
	};
...
uint8_t KeyboardPlusDriver::getModifier(uint8_t code) {
	switch (code) {
...
uint8_t KeyboardPlusDriver::getMultimedia(uint8_t code) {
	switch (code) {
...
bool KeyboardPlusDriver::process(Gamepad * gamepad) {
	...
	if ( keyboardReport.reportId == KEYBOARD_KEY_REPORT_ID ) {
		...
		keyboard_report_size = sizeof(KeyboardPlusKeyReport::keycode);
	} else {
		keyboard_report_payload = (void *)&keyboardReport.multimedia;
		keyboard_report_size = sizeof(KeyboardPlusKeyReport::multimedia);
	}

	bool keyboard_sent = false;
	if (keyboard_report_size != last_report_size ||
			memcmp(last_report, keyboard_report_payload, keyboard_report_size) != 0) {
		if (tud_hid_ready()) {
	...
	// Update and send analog axis report
	axisReport.lx = static_cast<uint8_t>(gamepad->state.lx >> 8);
	axisReport.ly = static_cast<uint8_t>(gamepad->state.ly >> 8);
	axisReport.rx = static_cast<uint8_t>(gamepad->state.rx >> 8);
	axisReport.ry = static_cast<uint8_t>(gamepad->state.ry >> 8);

	bool axis_sent = false;
	if (memcmp(last_axis_report, &axisReport, sizeof(axisReport)) != 0) {
		if (tud_hid_ready()) {
			if ( tud_hid_report(KEYBOARD_ANALOG_REPORT_ID, &axisReport, sizeof(axisReport)) ) {
				memcpy(last_axis_report, &axisReport, sizeof(axisReport));
				axis_sent = true;
			}
		}
	}

	return keyboard_sent || axis_sent;
}

void KeyboardPlusDriver::pressKey(uint8_t code) {
...
void KeyboardPlusDriver::releaseAllKeys(void) {
...
uint16_t KeyboardPlusDriver::get_report(uint8_t report_id, hid_report_type_t report_type, uint8_t *buffer, uint16_t reqlen) {
	if ( report_id == KEYBOARD_KEY_REPORT_ID ) {
		memcpy(buffer, (void*) keyboardReport.keycode, sizeof(KeyboardPlusKeyReport::keycode));
		return sizeof(KeyboardPlusKeyReport::keycode);
	} else if ( report_id == KEYBOARD_MULTIMEDIA_REPORT_ID ) {
		memcpy(buffer, (void*) &keyboardReport.multimedia, sizeof(KeyboardPlusKeyReport::multimedia));
		return sizeof(KeyboardPlusKeyReport::multimedia);
	} else if ( report_id == KEYBOARD_ANALOG_REPORT_ID ) {
		memcpy(buffer, (void*) &axisReport, sizeof(axisReport));
		return sizeof(axisReport);
	}
	return 0;
}

void KeyboardPlusDriver::set_report(uint8_t report_id, hid_report_type_t report_type, uint8_t const *buffer, uint16_t bufsize) {}

bool KeyboardPlusDriver::vendor_control_xfer_cb(uint8_t rhport, uint8_t stage, tusb_control_request_t const *request) {
    return false;
}

const uint16_t * KeyboardPlusDriver::get_descriptor_string_cb(uint8_t index, uint16_t langid) {
	const char *value = (const char *)keyboard_plus_string_descriptors[index];
	return getStringDescriptor(value, index);
}

const uint8_t * KeyboardPlusDriver::get_descriptor_device_cb() {
    return keyboard_plus_device_descriptor;
}

const uint8_t * KeyboardPlusDriver::get_hid_descriptor_report_cb(uint8_t itf) {
    return keyboard_plus_report_descriptor;
}

const uint8_t * KeyboardPlusDriver::get_descriptor_configuration_cb(uint8_t index) {
    return keyboard_plus_configuration_descriptor;
}

const uint8_t * KeyboardPlusDriver::get_descriptor_device_qualifier_cb() {
	return nullptr;
}

uint16_t KeyboardPlusDriver::GetJoystickMidValue() {
	return HID_JOYSTICK_MID << 8;
}

void KeyboardPlusDriver::handleEncoder(GPEvent* e) {
