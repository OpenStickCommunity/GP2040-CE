...
static_assert(CFG_TUD_ENDPOINT0_SIZE == 8,
    "MAYFLASH_S5_EXACT_IDENTITY requires an 8-byte EP0.");
#endif
...
// hidd_open() only claims HID, so claim each audio interface (0-2) here or
// SET_CONFIGURATION and the console's SET_INTERFACE stall.
static uint16_t mayflashs5_open(uint8_t rhport,
    ...
    // Skip this interface's descriptors and endpoints; audio is never streamed.
...
// Current DualSense firmware report (0x20). The console offers an update
// instead of proceeding when it sees the S5's older build. Needs updating as
// Sony ships newer controller firmware.
...
bool MayflashS5Driver::process(Gamepad * gamepad) {
    // S5 host transfers must run on core 0 with the TinyUSB host.
    if (mayflashS5AuthDriver != nullptr && mayflashS5AuthDriver->available()) {
        mayflashS5AuthDriver->process();
    }
    ...
    if (ps5AuthData->hash_ready) {
        ...
        if (tud_hid_ready() && tud_hid_report(0, ps5AuthData->send_hid_buffer, PS5_PACKET_SIZE) == true ) {
            ...
            if (reports_since_mount < CONSOLE_READY_REPORTS) {
                reports_since_mount++;
            }
            timeout_report_us = getMicro();
            upstream_dropping = false;
            upstream_wait_since_us = 0;
        } else if (tud_hid_ready()) {
            // Ready but refused: keep the report and retry next loop.
            return false;
        } else if (!tud_mounted()) {
            // No USB host: the dongle delivers to the console over Bluetooth, so
            // discard the report and keep feeding the dongle.
            ps5AuthData->hash_ready = false;
            timeout_report_us = getMicro();
        } else {
            // Console not reading USB. Once it moves to the dongle's Bluetooth link
            // it never will, so drop the report and keep feeding the dongle. Allow
            // longer while enumerating; once dropping, drop immediately.
            const uint64_t now_us = getMicro();
            if (upstream_wait_since_us == 0) {
                upstream_wait_since_us = now_us;
            }
            const uint64_t waited_us = now_us - upstream_wait_since_us;
            if (reports_since_mount < CONSOLE_READY_REPORTS ?
                    waited_us < ENUMERATION_DROP_US :
                    (!upstream_dropping && waited_us < UPSTREAM_DROP_US)) {
                return false;
            }
            ps5AuthData->hash_ready = false;
            upstream_dropping = true;
            upstream_wait_since_us = 0;
        }
    }
...
// Audio interfaces 0-2. The console reads and writes volume during
// enumeration and auth, so these must answer rather than stall.
    ...
    if (request->bmRequestType_bit.type == TUSB_REQ_TYPE_STANDARD) {
        switch (request->bRequest) {
            case TUSB_REQ_SET_INTERFACE:
                // Streams never start; just acknowledge the alternate setting.
                if (itf < 3) {
...
bool MayflashS5Driver::controlXferCb(
        uint8_t rhport, uint8_t stage,
        tusb_control_request_t const *request) {
    // Audio interface requests go to the audio handler.
    ...
    // Feature GETs are proxied live to the dongle.
    const bool is_feature_get =
        ...
        // 0x81 is stalled locally, as the dongle does.
...
void MayflashS5Driver::serviceDeferredControl() {
    ...
    // Pass the dongle's stall on to the console.
    ...
    // Substitute a current firmware build (see dualsense_current_0x20).
...
void MayflashS5Driver::queueConsoleSet(uint8_t report_id,
    ...
    // Forward only the writes this path uses, so a firmware update payload
    // never reaches the S5.
    switch (report_id) {
        case PS5AuthReport::PS5_SET_BLUETOOTH:    // 0x08
        case PS5AuthReport::PS5_SET_TEST_PARAM:   // 0x80
        // 0x0A pairing data and 0x08 are forwarded unchanged; the console
...
void MayflashS5Driver::set_report(uint8_t report_id, hid_report_type_t report_type, uint8_t const *buffer, uint16_t bufsize) {
    ...
    switch( report_id ) {
        case PS5AuthReport::PS5_SET_BLUETOOTH:
            ...
            // 0x08: Bluetooth on (1) / off (2), forwarded verbatim.
        ...
        case PS5AuthReport::PS5_SET_AUTH_PAYLOAD:
            // Count every F0 before validating it.
            ...
            // The PS button can trigger authentication. Pending gameplay
            // input must never discard a valid console challenge.
            ...
            break;
        default:
            // Everything else, including 0x0A pairing data, is forwarded untouched.
            queueConsoleSet(report_id, buffer, bufsize);
            break;
    };
}
