...
void XBOneAuthUSBListener::report_received(uint8_t dev_addr, uint8_t instance, uint8_t const* report, uint16_t len) {
    ...
    // Once authentication starts, relay complete USB transfers unchanged. In
    // particular, ACK packets belong to the console/controller exchange and
    // must not be consumed or regenerated by GP2040-CE.
    if ( xboxOneAuthData->auth_passthrough ) {
        xboxOneAuthData->dongleBuffer.setBuffer(report, len);
...
void XBOneAuthUSBListener::process_report_queue() {
    ...
    if ( mounted == true && !report_queue.empty() &&
        (xboxOneAuthData->auth_passthrough || (now - lastReportQueue) > REPORT_QUEUE_INTERVAL) ) {
