...
bool tud_vendor_control_xfer_cb(uint8_t rhport, uint8_t stage,
	...
	return DriverManager::getInstance().getDriver()->control_xfer_cb(rhport, stage, request);
}
...
uint16_t const *tud_descriptor_string_cb(uint8_t index, uint16_t langid) {
	return DriverManager::getInstance().getDriver()->get_device_string(index, langid);
}

// Invoked when received GET DEVICE DESCRIPTOR
// Application return pointer to descriptor
uint8_t const *tud_descriptor_device_cb() {
	return DriverManager::getInstance().getDriver()->get_device_descriptor();
}
...
uint8_t const *tud_hid_descriptor_report_cb(uint8_t itf) {
	return DriverManager::getInstance().getDriver()->get_report_descriptor(itf);
}
...
uint8_t const *tud_descriptor_configuration_cb(uint8_t index) {
	return DriverManager::getInstance().getDriver()->get_config_descriptor(index);
}

uint8_t const* tud_descriptor_device_qualifier_cb() {
	return DriverManager::getInstance().getDriver()->get_device_qualifier();
}
