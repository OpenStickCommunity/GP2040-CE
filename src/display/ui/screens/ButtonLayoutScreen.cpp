...
void ButtonLayoutScreen::updateCustomHeaders()
{
    ...
    if (prevSpecialMoveProfileNumber != profileNumber) {
        ...
        bannerMessage.assign(optionsProto.profiles[profileNumber].Label, strlen(optionsProto.profiles[profileNumber].Label));
        if (profileStr.empty()) {
            bannerMessage = "Special Profile #";
            bannerMessage +=  std::to_string(profileNumber);
        } else {
            bannerMessage.insert(bannerMessage.begin(), (21-bannerMessage.length())/2, ' ');
        }

        addCustomHeader(bannerMessage, "special");
    }*/
