...
const pinDirections = Object.entries(PIN_DIRECTIONS)
	.map(([key, value]) => ({
		label: key,
		value,
	}));
...
type ExpansionPinsFormTypes = {
    savePins: () => void;
    pins: { [key: string]: [ { [key: string]: { option: PinActionValues, direction: PinDirectionValues } } ] };
    setPinAction: (expansion: string, index: number, pin: string, action: PinActionValues) => void;
    setPinDirection: (expansion: string, index: number, pin: string, dir: PinDirectionValues) => void;
    expansionType: string;
};

const ExpansionPinsForm = ({ savePins, pins, setPinAction, setPinDirection, expansionType }: ExpansionPinsFormTypes) => {
