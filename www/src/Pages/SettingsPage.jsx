...
const HOTKEY_ACTIONS = [
	...
	{ labelKey: "hotkey-actions.cycle-pin-profiles", value: 22 },
	{ labelKey: "hotkey-actions.load-pin-profile-1", value: 15 },
	{ labelKey: "hotkey-actions.load-pin-profile-2", value: 16 },
	{ labelKey: "hotkey-actions.load-pin-profile-3", value: 17 },
	{ labelKey: "hotkey-actions.load-pin-profile-4", value: 18 },
...
const schema = yup.object().shape({
	...
	pinProfileNumber: yup.number().required().label("Pin Profile"),
});
...
const FormContext = ({ setButtonLabels }) => {
	...
	useEffect(() => {
		...
		if (!!values.pinProfileNumber)
			values.pinProfileNumber = parseInt(values.pinProfileNumber);
						...
						<Form noValidate onSubmit={handleSubmit}>
							<Section title={t("SettingsPage:settings-header-text")}>
								...
								<Form.Group className="row mb-3">
									...
									<div className="col-sm-3">
										<Form.Select
											name="pinProfileNumber"
											className="form-select-sm"
											value={values.pinProfileNumber}
											onChange={handleChange}
											isInvalid={errors.pinProfileNumber}
										>
											{[1, 2, 3, 4].map((i) => (
												<option
													key={`button-pinProfileNumber-option-${i}`}
													value={i}
												>
													{i}
												</option>
